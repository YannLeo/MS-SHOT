...
class BasicTrainer(_Trainer_Base):
    ...
    def _prepare_models(self, info):
        ...
        args_model = info['model']['args']
        self.model = self._get_object(models, info['model']['name'], args_model)
        self._resuming_model(self.model)  # Prepare for resuming models
        self.model = self.model.to(self.device)
    
    def _prepare_opt(self, info):
        self.opt = torch.optim.Adam(params=self.model.parameters(), lr=info['lr_scheduler']['init_lr'],weight_decay=0.01)
