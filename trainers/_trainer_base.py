...
font_songti = font_manager.FontProperties(fname="/usr/share/fonts/SourceHanSerifSC-Medium.otf")
...
class _Trainer_Base(ABC):
    ...
    def _plot_confusion_matrix(photo_path, labels, predicts, classes, normalize=True, title='Confusion Matrix',
        ...
        # plt.ylabel(u'真实标签', fontsize=FONT_SIZE+20, fontproperties=font_songti)
        # plt.xlabel(u'预测结果', fontsize=FONT_SIZE+20, fontproperties=font_songti)
        plt.savefig(photo_path, format="png", bbox_inches='tight', dpi=300)
    ...
    def _save_model_by_test_loss(self, epoch, valid_loss) -> bool:
        ...
        if valid_loss < self.min_valid_loss:
            ...
            if epoch % self.save_period == 0: # (__period := self.save_period-1) 
